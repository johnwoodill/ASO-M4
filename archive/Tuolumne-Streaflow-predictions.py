...
https://waterdata.usgs.gov/monitoring-location/11290000/#parameterCode=00065&period=P7D&showMedian=true

# Hetch Hetch only available from 2020 onward
"11290000"
...
# dat1 = pivoted_df[(pivoted_df['month'] >= 8)]
...
tdat = pd.read_parquet("data/Tuolumne_Watershed/predictions/NN-ASO-SWE-2010-2021_Feb01_new.parquet")
tdat['lat_lon'] = tdat['lat'].astype(str) + "_" + tdat['lon'].astype(str)
ttdat = tdat.groupby(['year']).agg({'tmean': np.sum, 'ppt': np.sum, 'swe_pred': np.sum}).reset_index()
...
# Bind all together
sdat['year'] = sdat['year'].astype(str)
ttdat['year'] = ttdat['year'].astype(str)

mdat = sdat.merge(ttdat, left_on=['year'], right_on=['year'], how='left')
# mdat = adat.merge(mdat, left_on=['year'], right_on=['year'], how='left').dropna().reset_index(drop=True)
mdat = mdat[['year', 'kaf', 'tmean', 'ppt', 'swe_pred']].dropna().reset_index(drop=True)
